...
import { allNumbersValid, FieldValidation } from "@/validation/user_fields";
...
export function validateImageData(data: AddImageCmd) {
  ...
  // regex csrf check
  const csrfValue = checkCsrf(data.csrf?.trim() ?? "");
  if (!csrfValue.isValid) {
    errors.csrf = errors.csrf ?? [];
    errors.csrf.push(...csrfValue.messages);
  }

  // validate title
  const checkTitle = checkImageTitle(data.title.trim());
  ...
  const checkDescription = checkImageDescription(data.description.trim());
...
export type UpdateImageActionCmd = {
...
// exists to send server error responses to the view/display
export type DeleteImageActionCmd = {
  errors: { [key: string]: string[] }; // Validation errors, if any
};

export type DeleteImageCmd = {
  csrf: string;
};
...
export function validateUpdateImageCmd(image: UpdateImageCmd): {
  ...
  // validate csrf token
  const csrfValue = checkCsrf(image.csrf?.trim() ?? "");
  if (!csrfValue.isValid) {
    errors.csrf = errors.csrf ?? [];
    errors.csrf.push(...csrfValue.messages);
  }
  ...
  // validate slug
  const slugValue = checkSlug(image.slug?.trim() ?? "");
  if (!slugValue.isValid) {
    errors.slug = errors.slug ?? [];
    errors.slug.push(...slugValue.messages);
  }

  // validate title
  const checkTitle = checkImageTitle(image.title.trim());
  if (!checkTitle.isValid) {
    errors.title = errors.title ?? [];
    errors.title.push(...checkTitle.messages);
  }
  ...
  const checkDescription = checkImageDescription(image.description.trim());
  if (!checkDescription.isValid) {
    errors.description = errors.description ?? [];
    errors.description.push(...checkDescription.messages);
  }
  ...
  if (image.album_slugs && image.album_slugs.length > 0) {
    image.album_slugs.forEach((slug) => {
      const slugValue = checkSlug(slug);
      if (!slugValue.isValid) {
        if (Object.keys(errors).length > 0) {
          errors.albums = errors.albums ?? [];
          errors.albums.push(
            `Album ${slug} has errors: ${Object.values(slugValue.messages)
              .flat()
              .join(", ")}`,
          );
        }
    ...
    image.permission_slugs.forEach((slug) => {
      ...
      if (!slugValue.isValid) {
        if (Object.keys(errors).length > 0) {
          errors.permissions = errors.permissions ?? [];
          errors.permissions.push(
            `Permission ${slug} has errors: ${Object.values(slugValue.messages)
              .flat()
              .join(", ")}`,
          );
        }
